...
import { TextLayer } from "./layers/texts/text-layer";
import { ShapeLayer } from "./layers/shapes/shape-layer";
import { ImageLayer } from "./layers/images/image-layer";
import { FrameLayer } from "./layers/frames/frame-layer";
        ...
        switch (e.key) {
          ...
          case "Backspace":
            ...
            if (state.selectedLayerIds.length > 0) {
              ...
              state.selectedLayerIds.forEach((id) => actions.deleteLayer(id));
            }
            return;
          default:
            return;
          ...
          state.selectedLayerIds.forEach((layerId) => {
            ...
            if (layer) {
              ...
              actions.updateLayer(layerId, {
                boxData: { ...layer.boxData, position: newPosition },
              });
            }
          });
        }
    ...
    >
      ...
      >
        {Object.entries(currentPage.layers as Record<string, Layer>)
          .sort(
            (a, b) => ((a[1] as any).zIndex || 0) - ((b[1] as any).zIndex || 0)
          )
          .map(([id, layer]) => {
            ...
            return (
              <div
                key={id}
                className={cn(
                  "absolute transition-shadow transition-transform duration-100",
              ...
              >
                ...
                {isSelected && (
                  <ResizeHandles
                    onResizeStart={(direction) => (e: React.MouseEvent) =>
