...
type Action =
  ...
  | { type: "UPDATE_LAYER"; payload: { layerId: string; props: Partial<Layer> } }
  ...
  return produce(state, (draft) => {
    switch (action.type) {
      ...
      case "SELECT_LAYER":
        ...
        break
      case "DESELECT_LAYER":
        draft.selectedLayerIds = draft.selectedLayerIds.filter((id) => id !== action.payload)
        break
      case "DESELECT_ALL":
      ...
      case "SET_SCALE":
        ...
        case "DELETE_LAYER": {
          ...
          if (draft.page?.layers[layerId]) {
            ...
            draft.selectedLayerIds = draft.selectedLayerIds.filter(id => id !== layerId)
...
export function EditorProvider({ children }: { children: ReactNode }) {
  ...
  const actions = {
    setSidebarTab: (tab: string | null) => dispatch({ type: "SET_SIDEBAR_TAB", payload: tab }),
    setSettingsOpen: (open: boolean) => dispatch({ type: "SET_SETTINGS_OPEN", payload: open }),
    selectLayer: (id: string) => dispatch({ type: "SELECT_LAYER", payload: id }),
    deselectLayer: (id: string) => dispatch({ type: "DESELECT_LAYER", payload: id }),
    deselectAllLayers: () => dispatch({ type: "DESELECT_ALL" }),
    setScale: (scale: number) => dispatch({ type: "SET_SCALE", payload: scale }),
    ...
    hoverLayer: (id: string | null) => dispatch({ type: "HOVER_LAYER", payload: id }),
  ...
  return <EditorContext.Provider value={{ state, dispatch, actions }}>{children}</EditorContext.Provider>
}
